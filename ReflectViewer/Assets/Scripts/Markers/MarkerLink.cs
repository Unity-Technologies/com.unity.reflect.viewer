...
namespace Unity.Reflect.Viewer
{
    ...
    public class MarkerLink : MonoBehaviour, IProjectLinkSource, IBarcodeDataParser
    {
        ...
        IEnumerator OpenMarkerBasedARCoroutine(string markerId)
        {
            ...
            var marker = m_MarkerController.MarkerStorage.Get(markerId);
            if (marker != null)
            {
                Debug.Log($"Queuing marker {marker.Value.Name} for alignment");
                m_MarkerController.QueuedMarker = marker.Value;
            }
        ...
        public bool TryParse(string inputData, out IMarker marker)
        {
            ...
            if (validUri)
                m_LoginManager.onDeeplink(inputData);
            marker = null;
            return validUri;
        }
