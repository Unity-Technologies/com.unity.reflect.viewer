...
report = BuildPipeline.BuildPlayer(
    ...
    [MenuItem("Tools/Vivox/AutoBuilder/Build Stadia")]
    public static void BuildStadiaMenu()
    {
        MenuButtonBuilds(BuildTarget.Stadia);
    }

    public static void BuildStadia()
    {
        HandleBuildConfig();

        BuildTarget target = BuildTarget.Stadia;
        if (!IsTargetGroupSupported(BuildTargetGroup.Stadia, target))
        {
            return;
        }
        ...
        report = BuildPipeline.BuildPlayer(
            BuildConfig.Levels,
            RELATIVE_BUILD_PATH + BuildConfig.Stadia.BuildPath + "/" + PlayerSettings.productName,
            target,
            BuildConfig.Stadia.shouldRunAfterBuild && isFromMenuPress ? BuildOptions.AutoRunPlayer : BuildOptions.None
        );
    ...
    public static void BuildSwitch()
    {
        ...
        report = BuildPipeline.BuildPlayer(
            BuildConfig.Levels,
            RELATIVE_BUILD_PATH + BuildConfig.Switch.BuildPath + "/" + PlayerSettings.productName + ".nspd",
            target,
