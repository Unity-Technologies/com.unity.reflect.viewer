...
namespace Unity.Properties.Tests
{
    class TypeConstructionTests
    {
        ...
        public void GettingAllConstructableTypes_FromGenericType_ReturnsProperCount()
        {
            ...
            TypeUtility.GetAllConstructableTypes<Types.NotConstructableBaseClass>(types);
            Assert.That(types.Count, Is.EqualTo(4));
            
            types.Clear();
            TypeUtility.GetAllConstructableTypes<Types.A>(types);
            Assert.That(types.Count, Is.EqualTo(1));
            
            types.Clear();
            TypeUtility.GetAllConstructableTypes<Types.NotConstructableDerivedClass>(types);
        ...
        {
            var types = new List<Type>();
            TypeUtility.GetAllConstructableTypes(typeof(Types.NotConstructableBaseClass), types);
            Assert.That(types.Count, Is.EqualTo(4));
            
            types.Clear();
            TypeUtility.GetAllConstructableTypes(typeof(Types.A), types);
            Assert.That(types.Count, Is.EqualTo(1));
            
            types.Clear();
            TypeUtility.GetAllConstructableTypes(typeof(Types.NotConstructableDerivedClass), types);
            Assert.That(types.Count, Is.EqualTo(0));
        }
        ...
        {
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<IConstructInterface>(), Is.True);
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<AbstractConstructibleBaseType>(), Is.True);
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<ConstructibleBaseType>(), Is.True);
        }
        
        [Test]
        public void CanBeConstructedFromDerivedType_FromNonConstructableDerivedType_ReturnsFalse()
        {
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<ConstructibleDerivedType>(), Is.False);
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<NonConstructibleDerivedType>(), Is.False);
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<NoConstructorType>(), Is.False);
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<ParameterLessConstructorType>(), Is.False);
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<ParameterConstructorType>(), Is.False);
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<ScriptableObjectType>(), Is.False);
        }
        ...
        {
            Assert.That(TypeUtility.CanBeConstructedFromDerivedType<object>(), Is.True);
